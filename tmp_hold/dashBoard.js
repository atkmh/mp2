...
getFiles("./results").then(function(ff) {
    ...
    stream.once("open", function(fd) {
        ...
        fs.readFile('./BUILD_TAG_NUM.txt', 'utf8', function(err, contents) {
            if (err) {              
                var buildno = '';
            } else {
                if (contents.length >= 1) {

                    var buildno = " - For Build :" + contents;
                } else {
                    var buildno = '';
                }
            }

//            var logo = '<table style="width:100%;position: absolute;top: 0%;"><tr><td width:10%"><img src="http://flossdental.com/wp-content/uploads/2017/10/delta-dental-plans-association_logo_5829-1024x205.jpg" height="100" width="180"></img></td><td width:70%><h4 style="font-family: Georgia; color:#3366CC;">TEST AUTOMATION RESULTS' + buildno + '<h4><h3 style="font-family: Arial; color:#AF7AC5;">' + 'LAST UPDATED AT:- ' + (moment().format('lll')).toString().toUpperCase() + '</h3></td><td width:10%"><img src="https://www.hcltech.com/sites/default/files/styles/large/public/images/guideline_based1.png?itok=JdASqCkG" height="100" width="180"></img></td></tr>';
            var logo = '<table style="width:100%;position: absolute;top: 0%;"><td width:70%><h4 style="font-family: Georgia; color:#3366CC;">TEST AUTOMATION RESULTS' + buildno + '<h4><h3 style="font-family: Arial; color:#AF7AC5;">' + 'LAST UPDATED AT:- ' + (moment().format('lll')).toString().toUpperCase() + '</h3></td></tr>';
            var html =
                "<!DOCTYPE html>" +
                "<html>" +
                "<head>" +
                ...
                "<tr><td style='border: 1px solid black' bgcolor=#D8D8D8><font size='2'>Internal Duration Time: </td><td id='internalduration' style='border: 1px solid black'><font size='2'>" + ((moment.duration(internalexecutiontime * 1000).minutes()) + ((moment.duration(internalexecutiontime * 1000).seconds()) / 60)) + ' Min' + "</td></tr>" +
